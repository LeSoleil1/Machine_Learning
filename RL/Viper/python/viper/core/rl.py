...
def _sample(obss, acts, qs, max_pts, is_reweight):
    print("obss shape",obss.shape,"acts shape",acts.shape,"qs shape",qs.shape)
    assert len(obss) == len(acts)  and len(obss) == len(qs)

    # # Step 1: Compute probabilities
    # ps = np.max(qs, axis=1) - np.min(qs, axis=1)
    # # print(type(ps),ps.shape,ps[110].shape) # <class 'numpy.ndarray'> (15848,) ()
    # ps = ps / np.sum(ps)

    #SOHEIL#################################################################
    # INSTEAD OF MAXIMUM MARGIN WE GO FOR THE MARGIN
    # qsorted = np.sort(qs, axis=1)
    ps =np.sort(qs, axis=1)[:,-1] - np.sort(qs, axis=1)[:,-2]
    ps = ps / np.sum(ps)
    ########################################################################
...
def train_dagger(env, teacher, student, state_transformer, max_iters, n_batch_rollouts, max_samples, train_frac, is_reweight, n_test_rollouts):
    ...
    # print(type(teacher.predict_q(np.array([obs for obs, _, _ in trace]))),teacher.predict_q(np.array([obs for obs, _, _ in trace])).shape,type(teacher.predict_q(np.array([obs for obs, _, _ in trace]))[110,0]),type(teacher.predict_q(np.array([obs for obs, _, _ in trace]))[110,1]),type(teacher.predict_q(np.array([obs for obs, _, _ in trace]))[110,2]),type(teacher.predict_q(np.array([obs for obs, _, _ in trace]))[110,3]),type(teacher.predict_q(np.array([obs for obs, _, _ in trace]))[110,4]),type(teacher.predict_q(np.array([obs for obs, _, _ in trace]))[110,5]))
    ...
    for i in range(max_iters):
        ...
        # print("obss type",type(obss),"acts type",type(acts),"qs type",type(qs))
        ...
        # print("obss type",type(cur_obss),cur_obss.shape,"acts type",type(cur_acts),cur_acts.shape,"qs type",type(cur_qs),cur_qs.shape)
        log('Training student with {} points'.format(len(cur_obss)), INFO)
        student.train(cur_obss, cur_acts, train_frac,i) #SOHEIL: i ADDED TO STORE ALL MODELS
    ...
    save_fname = 'dt_policy_Final.pk'
    save_dirname = '../tmp/pong/'
    f = open(save_dirname + save_fname, 'wb')
    pk.dump(max_student.return_Tree(),f)
    f.close()
